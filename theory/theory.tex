...
In the blowout regime the acceleration gradient is to a good degree linear in $\xi$ with
\begin{equation}
\label{eqn:linear_blowout_Ez}
E_\mathrm{z}(\xi)=\frac{1}{2}\kP\xi.
\end{equation}
One should keep in mind that the origin of the coordinate system has been shifted so that $\xi=0$ is at the zero-crossing of the electric field.
Integrating erquation \ref{eqn:linear_blowout_Ez} gives the linear potential
\begin{equation}
\label{eqn:linear_blowout_Pot}
U_\mathrm{z}(\xi)=\kP\xi^2.
\end{equation}
Now equation \ref{eqn:linear_blowout_Pot} is inserted in equation \ref{eqn:Trapping_Potential_TH} and rearranged in order to find for an electron released at an inital position $\xi_\mathrm{i}$ its trapping position $\xi$ by deriving
\begin{align}
U_\mathrm{z}(\xi_\mathrm{i})-U_\mathrm{z}(\xi_\mathrm{f})&=-\frac{\mE c^2}{\qE}\\
\kP(\xi_\mathrm{i}^2-\xi_\mathrm{f}^2)&=-\frac{\mE c^2}{\qE}\\
\label{eqn:Trapping_Position}
\xi_\mathrm{trap}&=\sqrt{\xi_\mathrm{i}^2-\frac{\mE c^2}{\qE \kP}}.
\end{align}
...
This finding can also be used to calculate the compression of a released electron beam during the trapping.
...
f(\xi)=\frac{\sigma_\xi}{\sqrt{2\pi}}e^{-\frac{\xi^2}{2\sigma_\xi^2}}.
\end{equation}
The distribution should be normalized to
\begin{equation}
\int_{-\infty}^{\infty}f(\xi)d\xi\stackrel{!}{=}1.
\end{equation}
Equation \ref{eqn:xi_init_dist} is now 
%\begin{align}
%f^\mathrm{trap}(\xi)&=f(\xi_\mathrm{trap}(\xi_\mathrm{i}))\\
%&=\frac{\sigma_\xi}{\sqrt{2\pi}}e^{-\frac{\xi^2}{2\sigma_\xi^2}}
