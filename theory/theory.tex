...
\subsubsection*{Plasma density profiles from laser ionization}
...
A good description of linear beam optics can be found in \cite{Wiedemann_accelerator} or \cite{Book_Fundamentals_Rosenzweig}.
...
%Injection of background plasma electrons into the wake in PWFA is not straightforwardly easy, because of the normally high plasma wave phase velocity $v_\phi=v_\mathrm{bunch}\approx c$ close to the speed of light., 
%\cite{KatsouleasPRL1988WaveBreak}.
...
In contrast to these hydrodynamic methods we have investigated the optical generation of a plasma density spike, also called \textit{Plasma Torch} \cite{PlaTo,PlaTo2} as means for an electron bunch injection.
...
In \cite{PlaTo} and \cite{PlaTo2} we used FACET-like driver bunches 
